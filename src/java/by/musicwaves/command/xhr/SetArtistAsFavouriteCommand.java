...
/**
 * This XHRProcessor is designed to set single artist as favourite. <br><br>
 * Access restrictions: users that haven't completed registration 
 * (NOT_REGISTERED_USER level users) are not permitted to use it
 */
public class SetArtistAsFavouriteCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("SET ARTIST AS FAVOURITE command reached");
        ...
        try
        {
            ...
            {
                dao.setArtistAsFavourite(user.getId(), artistId);
            ...
            {
                LOGGER.error("SET ARTIST AS FAVOURITE command, error occured", ex);
