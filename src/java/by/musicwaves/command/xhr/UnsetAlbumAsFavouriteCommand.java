...
/**
 * This XHRProcessor is designed to un-set single album as favourite. <br><br>
 * Access restrictions: users that haven't completed registration 
 * (NOT_REGISTERED_USER level users) are not permitted to use it
 */
public class UnsetAlbumAsFavouriteCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("UN-SET ALBUM AS FAVOURITE command reached");
        ...
        try
        {
            ...
            {
                dao.unsetAlbumAsFavourite(user.getId(), albumId);
            ...
            {
                LOGGER.error("UN-SET ALBUM AS FAVOURITE command, error occured", ex);
