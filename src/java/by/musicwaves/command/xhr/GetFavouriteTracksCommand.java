...
/**
 * This XHRProcessor is designed to get list of tracks set by user as favourite 
 * + bound albums and artists data, and return it as JSON-object to the requesting side. <br><br>
 * Data activity state: ACTIVE ONLY <br>
 * Access restrictions: any level users, login required.
 */
public class GetFavouriteTracksCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("GET FAVOURITE TRACKS command reached");
        ...
        try
        {
            ...
            {
                List<List<Map<String, String>>> queryResult = dao.getFavouriteTracksData(user.getId(), "%" + searchString + "%", limit, offset);
            ...
            {
                LOGGER.error("GET FAVOURITE TRACKS command, error occured", ex);
