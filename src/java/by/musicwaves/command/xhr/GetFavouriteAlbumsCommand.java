...
/**
 * This XHRProcessor is designed to get list of albums set by user as favourite 
 * + bound artists data, and return it as JSON-object to the requesting side. <br><br>
 * Data activity state: ACTIVE ONLY <br>
 * Access restrictions: any level users, login required.
 */
public class GetFavouriteAlbumsCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("GET FAVOURITE ALBUMS command reached");
        ...
        try
        {
            ...
            {
                List<List<Map<String, String>>> queryResult = dao.getFavouriteAlbumsData(user.getId(), "%" + searchString + "%", limit, offset);
            ...
            {
                LOGGER.error("GET FAVOURITE ALBUMS command, error occured", ex);
