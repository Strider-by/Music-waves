...
/**
 * This XHRProcessor is designed to set single audio track as favourite. <br><br>
 * Access restrictions: users that haven't completed registration 
 * (NOT_REGISTERED_USER level users) are not permitted to use it
 */
public class SetAudiotrackAsFavouriteCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("SET TRACK AS FAVOURITE command reached");
        ...
        try
        {
            ...
            {
                dao.setTrackAsFavourite(user.getId(), trackId);
            ...
            {
                LOGGER.error("SET TRACK AS FAVOURITE command, error occured", ex);
