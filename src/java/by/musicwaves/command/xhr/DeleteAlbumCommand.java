...
/**
 * This XHRProcessor is designed to delete existing Album instance. <br><br>
 * All dependent data, both database-held records (audiotracks records, 
 * favourite audiotrack records, favourite album records, playlist item records)
 * and files (album image, audiotracks files) will be deleted as well. <br><br>
 * Access restrictions: ADMINISTRATOR and CURATOR level users only.
 */
public class DeleteAlbumCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("DELETE ALBUM command reached");
        ...
        try
        {
            String idString = request.getParameter(PARAM_NAME_ALBUM_ID);
            int id = Integer.parseInt(idString);

            try (CrossEntityDao crossEntityDao = new CrossEntityDao())
            {
                // getting tracks being deleted file names, album image,
                // delete album and tracks records,
                // deleting tracks files, album image
                // deleting faved items and playlist items

                List<List<Map<String, String>>> queryResult = crossEntityDao.deleteAlbum(id);
                if (queryResult == null || queryResult.size() != 2)
                {
                    throw new CommandException("query result does not meet expectations");
                }

                List<String> albumsImagesNeededToBeDeletedFileNames = getImagesNames(queryResult.get(0));
                List<String> tracksNeededToBeDeletedFileNames = getTracksNames(queryResult.get(1));
            ...
            {
                LOGGER.error("DELETE ALBUM command, error occured", ex);
