...
/**
 * This XHRProcessor is designed to un-set single audio track as favourite. <br><br>
 * Access restrictions: users that haven't completed registration 
 * (NOT_REGISTERED_USER level users) are not permitted to use it
 */
public class UnsetTrackAsFavouriteCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("UN-SET TRACK AS FAVOURITE command reached");
        ...
        try
        {
            ...
            {
                dao.unsetTrackAsFavourite(user.getId(), trackId);
            ...
            {
                LOGGER.error("UN-SET TRACK AS FAVOURITE command, error occured", ex);
