...
/**
 * This XHRProcessor is designed to get single artist data and return it 
 ...
 */
public class GetArtistByIdCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("GET ARTIST BY ID command reached");
        ...
        try
        {
            ...
            try (ArtistDao dao = new ArtistDao())
            {
                ...
                if (artist == null)
                {
                    LOGGER.debug("We failed to find such an artist");
            ...
            {
                LOGGER.error("GET ARTIST BY ID command, error occured", ex);
