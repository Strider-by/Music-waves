...
/**
 * This XHRProcessor is designed to set single album as favourite. <br><br>
 * Access restrictions: users that haven't completed registration 
 * (NOT_REGISTERED_USER level users) are not permitted to use it
 */
public class SetAlbumAsFavouriteCommand implements XHRProcessor
{
    ...
    {
        LOGGER.debug("SET ALBUM AS FAVOURITE command reached");
        ...
        try
        {
            ...
            {
                dao.setAlbumAsFavourite(user.getId(), albumId);
            ...
            {
                LOGGER.error("SET ALBUM AS FAVOURITE command, error occured", ex);
